...
public class ModbusRTUTransport extends ModbusSerialTransport {
	...
	public void writeMessage(ModbusMessage msg) throws ModbusIOException {
		try {
			...
			synchronized (m_ByteOut) {
				...
				//MainActivity.ShowToastMessage("Data to write: " + msg.getHexMessage());
				...
				//MainActivity.ShowToastMessage("Data to write: " + m_OutputStream.toString() + " " + HexDump.dumpHexString(buf, 0, len));
	...
	/**
	 * Очистит ввод, если во входном потоке обнаружены символы.
	 * 
	 * @throws IOException
	 */
			...
			do {
				...
				synchronized (m_ByteIn) {
					...
					if (uid != -1) {
						...
						// создать ответ, чтобы узнать длину сообщения
						response = ModbusResponse.createModbusResponse(fc);
						response.setHeadless();

						// С Modbus RTU нет конца фрейма. Либо мы предполагаем, что сообщение
						// завершено как есть, либо мы должны выполнить специфичную для функций
						// обработку, чтобы узнать правильную длину.
						// Чтобы избежать переноса времени фрейма на функции последовательного ввода,
						// мы устанавливаем тайм-аут и анализируем сообщение, чтобы прочитать ответ.
						Log.d("Response 1", response.getHexMessage());
						getResponse(fc, m_ByteInOut);
						dlength = m_ByteInOut.size() - 2; // без CRC
						if (Modbus.debug)
						...
						Log.d("Response 2", response.getHexMessage());
						// check CRC
						...
						if (ModbusUtil.unsignedByteToInt(m_InBuffer[dlength]) != crc[0]
							...
							throw new IOException(
									"CRC Error in received frame: "
											...
											+ crc[1]
											+ "Received CRC :"
											+ ModbusUtil.unsignedByteToInt(m_InBuffer[dlength])
											+ ModbusUtil.unsignedByteToInt(m_InBuffer[dlength + 1]));
						}

						Log.d("Response 3", "CheckCRC");
					} else {
					...
					Log.d("Response 4", response.getHexMessage());
					done = true;
					Log.d("Response 5", String.valueOf(done));
				}// synchronized
			} while (!done);
			return response;
		} catch (Exception ex) {
